 ...
 */
public class ResetPanel extends SettingPanel {
	/**
	 * パスワード入力用
	 */
	 ...
	 * @param message 設定の説明
	 */
	public ResetPanel(ChangeSettingDialog csd, String message) {
		super(message);
		this.csd = csd;
	...
	protected boolean changeProcessing() {
		...
		new YesNoDialog(csd, "本当にリセットしますか？", setting.getSettingFont(0)) {
			...
			protected void addLayoutProcessing() {
				// 誤作動防止でEnterキーの入力を無視する
				button1.removeKeyListener(this);
				button2.removeKeyListener(this);
			}
